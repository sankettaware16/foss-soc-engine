...
def check_redis(config, rules, timeout):
    ...
    if redis_lib is None:
        ...
        return 1
    # The engine connects to localhost:6379 (see core/engine.py).
    try:
        client = redis_lib.Redis(host="localhost", port=6379, db=0,
                                 socket_connect_timeout=timeout)
        client.ping()
        tc.report("OK", f"Redis reachable at localhost:6379 "
                  f"(needed by: {', '.join(stateful)})")
        return 0
    except Exception as e:
        tc.report("ERROR", f"stateful rules need Redis but localhost:6379 is "
                  f"unreachable ({e})")
        return 1
