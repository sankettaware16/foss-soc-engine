...
- **Rules in use** — every rule with how many events it parsed plus its
  no-match / buffered / error counts (instantly shows which parser is busy and
  which is failing).
...
- **Recent errors / DLQ** — click *Load latest* to see the most recent
  un-parseable lines and why they failed.
...
| Geo enrichment (country/city on IPs) | `geoip2` + an `.mmdb` file | quietly skipped (no geo fields) |
